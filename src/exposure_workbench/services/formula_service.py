...
async def evaluate_formula_series(db: AsyncSession, ticker: str, name: str, *,
    ...
    for leaf in leaves:
        ...
        # the candidate whose facts reach the latest period, not the first with any:
        # NVDA's `revenue` has three facts to 2022-01-30 and `total_revenues` runs to
        # 2026-07-26, and a days_sales_outstanding grid on the first stopped four
        # years short of the present.
