...
> **状态(2026-08-30)**:**S0–S6c ✅、S8 验收 ✅ 且已上线**。S0 共享面 · S1 数据卫生 · S2 错误面 · S3a 证据标签 · S3b `meta.verified` · S4a 工具 display · S5 11 个只读端点 · S6a 图表原语 · S6b 审计层/证据栏/分析师面板 · **S6c 两个页面重写完成**(dock/审计/证据栏挂在 layout;book 页与 issuer 页全部改用新组件;退休 ChatPanel / Evidence 抽屉 / RunTimeline)。**剩 S7 措辞**(LLM 路径一字未改,待逐条过目)。**1310 offline / tsc 绿 / next build 绿**(基线 1082)。**已推送并已部署**(四镜像重建,容器内 grep 核实)。**S8 判据实测通过**:`scripts/smoke_ui.py` 对线上跑 7 项全 PASS——5 个视图读者层**零** id、零供应商/内网串、零 JS 错误;打开审计层书页出现 11 个 id;审计开关**跨导航仍在**(这正是它必须挂 layout 的原因)。
