...
> **状态(2026-08-29)**:执行中。**S0 ✅** 共享面 + Next 16 笔记 · **S1 ✅** 数据卫生 · **S2 ✅** 错误面 · **S3a ✅** 证据标签 + 显示名表 · **S3b ✅** `meta.verified`(门保留匹配)· **S4a ✅** 工具 display + 审计层后端 · **S5 ✅** 11 个只读端点 · **S6a ✅** 图表原语 · **S6b ✅** 审计上下文、证据卡与证据栏、分析师面板、单遍答案渲染器(**组件层自洽,尚未接进两个页面**)· S6c 页面重组 / S7 措辞 / S8 验收 待做。**1309 offline / tsc 绿**(基线 1082)。**已推送**:`origin/issuer-intelligence` = `7c2a32f`(此前停在 `ec64079`,落后 10 个提交)。**已部署**:四个镜像重建 → `up -d`,切换期一次 502 后恢复 200;容器内 grep 核实运行镜像确含本批代码;11 个新端点线上匿名 200;`error_detail` 全线 0 次;S5 同源判据线上复验(history 末点 10,845,260.0 = run 存的 `portfolio_market_value`;`vol_30d` 0.135581 = `rolling_vol_30d` 0.13558095)。
...
> **续做入口**:`apps/web/app/components/{audit.tsx,analyst/,evidence/,charts/}` 已就位且自洽;下一步是 `app/layout.tsx` 挂 `AuditProvider`+`DockContextProvider`+`EvidenceProvider`+`AnalystDock`,再重写 `app/page.tsx` 与 `app/issuer/[ticker]/page.tsx` 使用这些组件与 `lib/charts.ts` 的端点。
