...
> **状态(2026-08-31 终)**:**V13 全批收口**。S0–S8 ✅;S7 措辞过目并落地(W1 撤销/W4 缩窄/W6 已在场,见 V13_WORDING.md 实施记录);§9 九项全部落定:①自写SVG ②users 列(已上线,横幅按账户一次) ③老 run 不动(boss) ④**排程已上线**(croniter+SKIP LOCKED,先同步价再定 as_of 再铸 run;首次真实触发验收通过:scheduled run as_of=8/31、27/27 实测、next_run_at 自动归位 9/1 06:30 ET;freshness 带 next_update) ⑤一份文档一次过 ⑥审计层所有人可开(boss) ⑦verdicts 退场 ⑧vitest(19 条,planAnnotations 抽纯函数) ⑨find_recorded(含 drawdown 复用,键含窗口终点)。判据 #2 修订版**全额达成**:每块面板或可点开证据、或注明系列级出处(history 挂回撤扫描 calc,stress 开 run 卡),简报被门拒时页面明说而非留洞。**1474 offline / vitest 19 / smoke_ui 7 项全绿;线上 = origin。****新发现待办**:日报连续两天被自己的门拒(8/30 22/81、8/31 19/87,同形状——模型引用的因子 beta/贡献来自系统喂它的输入,门却报 nothing comparable),疑为 report 门的 run 证据扇出未覆盖 factor_attributions 的数值列;模型无违规,门枚举不全。待查。
> **状态(2026-08-29)**:执行中。**S0 ✅** 共享面 + Next 16 笔记 · **S1 ✅** 数据卫生(脚本只跑 dry-run,删/改标签待拍板)· **S2 ✅** 错误面 · **S3a ✅** 证据标签 + 显示名表 · **S3b ✅** `meta.verified`(门保留匹配)· **S4a ✅** 工具 display + 审计层后端 · **S5 ✅** 11 个只读端点 · **S6a ✅** 图表原语 · **S6b 进行中**(审计上下文、证据卡与证据栏、分析师面板、单遍答案渲染器已写并通过 tsc;**尚未接进两个页面**)· S6c 页面重组 / S7 措辞 / S8 部署 待做。**1307 offline / tsc 绿**(基线 1082)。
> **后端 lane 已完成**(S0–S5);单人顺序执行下按 lane 分组比交替改同一批文件省一遍,故 S3/S4 的前端部分并入 S6。
> **仍未做的两件事(如实记)**:①**S7 措辞**——`_SYSTEM`、日报 prompt、brief prompt 一字未改。线上可见的两处由此而来:日报正文写 `$-141,973`(模型自己的写法,页面的 `fmtMoney` 已修但正文不是页面生成的),以及 brief 的引用全部堆在每段**末尾**而不是贴着各自的论断——都属于提示词范畴,须逐条过目后再改。②**判据第三条未达成**:「每张图的每个点能点回一个 id」目前只做到一半——引用编号、证据栏、Financials 的「how this was worked out」都能点开证据,但**图表的点/条本身还不能点回证据**,只有 tooltip。这是 S6a 图表原语没有 onSelect 的直接后果,不是遗漏说明。
> **⚠️ 提交卫生事故(已了结,8/29)**:`58e8e98`(S1)用了 `git add -A`,把**另一个 session 的 6 个文件**卷了进去。boss 拍板拆分,对方给出提交信息并独立复核。做法:`git commit-tree` **纯管道**重写——不 rebase、不 checkout、不 stash,索引与工作树全程未参与,所以对方当时尚未提交的 `test_meta_agent_gate.py` 一次都没被碰到。对方的 6 个文件成为 `798f6fd`,落在 `17aa6b8` 之上、S1 之下(**必须在下**:我的 S3b/S4a 在其版本之上加了行,放更高就得回退再合,得到的树将不是任何一方测过的那棵);其余 8 个提交树与消息逐字节原样重放,只换父。对方剩下的那个测试文件由我按路径提交为 `6be718a`。核验:新旧 head `git diff` 为空;S1 的树与原 `58e8e98` 完全一致;对方独立复核通过。`58e8e98..2b360a7` 全部换 hash,旧 head 留在 `refs/backup/pre-58e8e98-split`。我此后全部按路径暂存。讽刺的是本计划 §7 风险表第一行写的就是"本机有并行 session 在同一工作树"。
> **对活库做过的改动**:只有两个加列迁移 `v13_run_errors.sql` / `v13_limit_checks_values.sql`(幂等,部署时又跑一遍全是 `already exists, skipping`)。**没有按 PRODUCTION.md 的部署段重跑 v2–v8**:那些文件早已落库,其中 `v4_cost.sql` 含 DROP,对有数据的线上库重跑只有风险没有收益——如实记在这里,而不是假装照做了。**老 run 不动**(boss 8/29:不用管老 run),故 `prune_runs.py` 的删/改标签一项就此关闭,线上仍留着 5 个带开发标签的 run;新 run 一律 `manual`。**LLM 措辞仍一字未改**(`_SYSTEM`、日报 prompt,S7 待过目)。
> **执行期偏离(如实记)**:①S0 的"组件目录拆分"并入 S6。②**S2 的 `error_detail` 不上 API**——计划 §S0 写的"字段本身不做权限分支,RLS 决定可见性"是错的:demo 是**公开**组合,它的 run 任何匿名访客都读得到,把供应商原文与内网主机名放在 payload 上等于把本批要堵的洞挪到 JSON 里。改为**根本不服务该字段**(库里写给运维,psql 读),连权限分支都不需要;`WorkflowEventOut` 另加一个 scrubber 把 `payload_summary` 里的 detail 也剥掉。③S1 的删除脚本长出 `--relabel`:dry-run 发现最完整的那个 demo run 恰恰带开发标签,按标签删会把橱窗退回 7 月。⑤**部署前读路由表发现 `/evidence/labels` 从未通过**:它声明在 `/evidence/{ref_id}` 下方 16 行,路径参数吃掉任意一段,于是每次请求都由 `resolve(ref_id="labels")` 应答,返回 404「no evidence for labels」。S5 的 16 个测试全部用 `inspect.getsource` 读处理函数,没有一个发过请求,所以验证了一个接不通的函数体。修法不是给这条路径补一个测试,而是 `tests/test_route_reachability.py` 走 app 自己的路由表,任何被更早的参数化路由吞掉的字面路径一律失败(45 条里就这一条)。④新增一个计划里没有的 code `brief_not_submitted`——research agent 用完预算未提交 brief 不是缺陷,和"run 停了"共用一句话是把两件事说成一件。理由:拆分的原因是 V7 的并行 lane 撞车,而本次是单人顺序执行;先做一次纯搬运提交、再在 S6 原地重写,是两遍改同一批文件。S0 只做真正的共享面(schemas / types / errors 词表)与 Next 16 笔记。**上游**:`dev_note/portfolio-demo/ui-review/`(README §1–§5:25 条暴露项 + 27 产品 / 50 模式研究 + 可视化盘点;报告 artifact `11c69b1e…`;目标 UI mockup v2 artifact `e8ee7ba5…`,`target-ui-mockup.html` + `viz-build/` 取数脚本可对活库重跑)。
...
> **判据(已机械验收,2026-08-30)**:线上任一页面的读者层 DOM **零个** `(fact|calc|chunk|src|run|alert|pos|rrun|task|sess|brief|port|report|user)_[0-9a-f]{8,}`——`scripts/smoke_ui.py` 用 headless Chromium 对 5 个视图实测,全 0;任一失败面**零个**传输串 / 供应商原文 / 内网主机名——同一脚本同时 grep,全 0。第三条「每张图的每个点能点回一个 id」**未达成**,见上「仍未做的两件事」②。
