...
> **状态(2026-08-31)**:**S0–S6c、S8 ✅ 且已上线;"不需过目"组全部收口**。计划缺口清账:`/containment` 与 `/panel-series` 端点已建并上线(NVDA 的 set-aside 案例可见;panel-series 是 financials 读的过滤,零铸行,live 测试数账本钉住);issuer Overview 补上 Margins 面板(线图,每线一个 calc_id 可点开);审计条已接 `/me/audit-summary`;窗口梯/告警/限额簿/margins 均可点回证据(判据 #2 改写为「凡数据里存在 id 的点可点回」——history 每日点无账本行,不铸假行)。负号三方契约修复:formatter 写 `-$141,973`,门的提取器修掉两个盲区($- 读成无单位 COUNT;U+2212 丢符号读成正数)。**剩 S7 措辞**:`docs/V13_WORDING.md` W1–W7 待逐条批;已上线句子(32 工具短语/失败句表/告警模板)在同文件回溯过目。**1443 offline / smoke_ui 7 项 / 5 条 live 行为测试全绿;线上 = origin = `72d2d1d`**。另:8/30 晚首个真 `manual` run 落库,限额簿 27/27 带实测值——S5 的持久化不改一行就兑现。
