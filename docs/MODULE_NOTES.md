...
## M29 — 重建、两个复现、以及"没人走过的载荷形状"这一类的第四次(V29,2026-09-08/09,已提交已部署)

### 做了什么

四镜像重建(web 输入未变故同镜像),23 项 V27/V28 标记逐条在**容器里** grep 到;live 套件跑基线对比(改前 28 失败/改后 27,**新增 0**,修复 1;那 27 条全是 V23 删工具后的死测试,引用 `get_flow`/`_get_task_status`/`formulas`)。电池按 X26 条件跑**两个完整复现**(串行、书本已有 fresh completed run、经容器 MCP),每轮 191 题,0 异常。报告 `docs/spikes/v29/FINDINGS.md`,对比脚本 `scripts/v29_compare.py`,跑批脚本 `scripts/v29_run_battery.sh`。

### V27 目录成立(机械计数,9/7 串行 → 9/8 R1,第三列是同代码两轮方差)

方法名进 read_book **132/51轮 → 5/3轮**(5 vs 7);`unknown_method` 32 → **0**(0 vs 0);`unknown_series` 43 → **0**;`op_or_method` 43 → **0**(改由 schema 扣预算前拒);filings 崩适配器 21 → **0**;**打开单个域卡片 0(做不到)→ 172 次/127 轮**(119 vs 121);出图方法数 35/46 → **40/46**;`price.beta` 4ok/10fail → 9/2,`gross_margin` 5/2 → 10/0,`book.explain_episode` 零调用 → 7 次成功。`metric_not_filed` 26→34 是**反转**:9/7 那 26 次含 17 次方法名走错门 + 6 次编造名,9/8 的 34 次全是真实"未申报"。代价:根上 `expand` 被拒 91 次/~67 轮(占位符主体的必然结果),每轮多约 0.7 次调用,超预算轮数 8→9。

### 分数不可分辨(本轮最重要的方法论结论)

同代码两复现:总分 **325 vs 361 / 606(5.9%)**,而 9/7 基线 350 **落在两者之间**。R1 之后我曾标记"下降 25 分",那是仪器不是变化。逐判据只有五处基线在复现区间外:follows_on/honest_absence/trigger 改善,**precision(74→57,65)与 so_what(44→35,38)两轮都更差**,机制是能拿到更多真实数字就都写进答案而判据罚"账本级精度";是否对读者更差本轮定不了。预算不是原因,rank 成功轮数反而 7→11。

### 又四个缺陷,同一条边界:生产者知道单位,消费者按键名或形状猜

①`book.explain_episode` 崩在 `portfolio_window_return`——**244 轮电池里零调用**,载荷从没过过适配器,V27 把模型路由过去第一次就死;修法:方法的单位由 `skill.Method.unit_class` 声明一次,适配器读它,特定键仍优先。②`compute(method=[列表])` TypeError——①的修法自己引入,`method` 可以是列表。③集合统计的 `entries` 无声明单位——V25 的集合统计在 102 轮里零使用。④**`rank` 的 `spread` 是 max−min,单位随被排序的度量走**,`UNIT_BY_KEY` 钉成 RATIO 且键名清单优先于声明,于是 $1,135,470 被印成 **`113547000.0%`** 到读者面前;修法:`POLYMORPHIC_KEYS`(spread/difference/gap)只读生产者声明,未声明则拒不默认。守卫:live 扫描从"每个方法"扩到**每种 compute 形状 + 全部 12 个 op**;单元测试双向钉住多态单位。端到端复验:同一位置现在是 `$1.29M`。

### 还有一个把线上打挂的(V28-R 已记,此处并列因同属"契约只在一侧成立")

V28 A2 的顶层 JSON Schema `not`:jsonschema 接受、2191 条离线绿、**供应商整个函数 400**,电池 140 轮全 0 调用,desk-for-one.com chat 从部署到发现一直是坏的。改为 `registry.Shapes` 声明 + registry 扣预算前拒;守卫两条:结构测试(逐字编码供应商的规则)+ live 测试(真发一次给供应商)。

### 质量层(6 lens 排查 + 独立 verifier 对抗式核实:135 判决,**126 成立,9 被驳回**)

五类:(a)**新**——无量纲数一律印成百分比:`620.9% days`(实为 6.21 天)、`14583.3%`、P/E `3012.1%`、VIF `1661.9%`、`320.9% times`;根因是 `compute` 让模型命名结果却不能声明"这是几倍/几天",而渲染器把 RATIO 一律当百分比(MONEY÷MONEY_PER_DAY=COUNT 那条路是好的)。(b)段落标签占数字位(X10 未变)。(c)角色错误照旧:两次"前五大"又不是前五大、负的经营现金流被当成"在产生现金流"、增速方向读反、窗口长度当波动率读——**这是 9/1 门契约的边界,boss 未拍板**。(d)刚算出来又说"桌上没有"。(e)多轮 7–16 次成功调用后仍以"我无法给出可以担保的答案"收尾——**建议单独查**:是门拒得对而模型没适应,还是被拒后回合用尽。

### 三条纪律(比结论值钱)

①**离线绿不是契约**:凡发给外部服务的结构,必须有一条真发一次的守卫。②**"某能力零调用"不叫没人用,叫从未验证**——本轮四个缺陷里三个在上一轮电池从未走过的路线上。③**单位属于量,不属于键名**:同名键在不同结果里单位不同时,只有生产者能说,消费者必须读声明而不是猜;`UNIT_BY_KEY` 只留定义上固定的名字(`sessions` 永远是计数,回撤 `depth` 永远是比率)。
